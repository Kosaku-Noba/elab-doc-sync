...
> **Note:** pull 時にリモートのカテゴリ・タグが変更されていれば `.elab-sync.yaml` に自動反映されます（merge モードのみ）。each モードでリモートのタイトルが変更されていた場合、ローカルのファイル名も自動でリネームされます。リネーム先に同名ファイルが既に存在する場合はリネームをスキップし、旧ファイル名を維持します。
