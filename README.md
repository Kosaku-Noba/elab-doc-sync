...
> **Tip:** `--dir` がターゲットの `docs_dir` と異なる場合は一時エクスポート扱いとなり、同期状態（次回の push 差分判定や pull の再取得対象）には反映されません。複数の `docs_dir` が設定にあり `--dir` を省略した場合、対話的に保存先を選択するプロンプトが表示されます（非対話環境では `--dir` を明示指定してください）。
...
> **Note:** pull 時にリモートのカテゴリ・タグが変更されていれば `.elab-sync.yaml` に自動反映されます（merge モードのみ）。each モードでリモートのタイトルが変更されていた場合、ローカルのファイル名も自動でリネームされます。リネーム先に同名ファイルが既に存在する場合はリネームをスキップし、旧ファイル名を維持します。ローカルファイルを手動削除した場合は新タイトル名で再作成されますが、同名ファイルが既にあれば `--force` が必要です。
### 差分を確認（diff）
...
> **Note:** カテゴリは単一値で、push 時に上書きされます（タグの追記方式とは異なります）。タイトル・カテゴリ・タグのいずれかが変更されていれば、本文に変更がなくても push 対象になります。カテゴリ名は完全一致で解決されます。
...
同名・同サイズのファイルは再アップロードされません（リモートがハッシュ情報を返す場合は内容の差分も検知します）。内容を差し替えた場合は `--force` で強制送信:
...
| アップロードがタイムアウト | 自動で1回リトライされます。繰り返す場合はネットワークを確認 |
