...
elab-doc-sync は、ローカルの Markdown ドキュメントを eLabFTW の「アイテム」に同期する CLI ツールである。YAML 設定ファイルに基づき、複数のドキュメントセット（ターゲット）をそれぞれ独立した eLabFTW アイテムとして管理する。
...
targets:                  # 必須。1 件以上
  - title: <string>      # 必須。eLabFTW アイテムのタイトル
    docs_dir: <string>   # 必須。Markdown ファイルのディレクトリ
    pattern: <string>    # 任意。Glob パターン。デフォルト: "*.md"
    id_file: <string>    # 任意。アイテム ID 保存先。デフォルト: ".elab-sync-ids/default.id"
```
...
[My Project Docs] 変更あり
  ファイル: 3件  画像: 2件  → アイテム #42 を更新します
[API Reference] 変更なし（スキップ）
  ファイル: 1件  画像: 0件  → アイテム #43 を更新します
```

新規作成の場合:
```
[New Docs] 変更あり
  ファイル: 2件  画像: 1件  → 新しいアイテム を更新します
```
...
| `read_item_id()` | `int \| None` | 保存済みアイテム ID の読み込み |
| `save_item_id(item_id)` | `None` | アイテム ID をファイルに保存 |
