...
| `#{id} が見つかりません` | エンティティが削除された | 自動再作成しない。削除をサーバー側で確認後、`rm` → `link --new --file <文書名>` → pushで新規登録する |
...
競合時は `esync diff` で両側の編集を確認してください。通常のpullはローカル編集を保持します。明示的に採用する側を選ぶときだけ `pull --force` または `push --force` を使います。

作成結果不明、同期未完了、ローカル変更の中断、バックアップからの復元は [移行と復旧](13_MIGRATION_V1.md#途中失敗の復旧) を参照してください。
