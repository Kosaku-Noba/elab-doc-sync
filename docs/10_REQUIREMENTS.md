...
| 低 | FR-17 | マルチユーザー対応 | `whoami`、ロック機構 | ✅（whoami のみ） |
| 低 | FR-18 | テンプレート機能 | テンプレートから新規ドキュメント生成 | ✅ |
| 中 | FR-19 | リモート一覧表示 | `esync list` でリモートエンティティ一覧 | ✅ |
| 中 | FR-20 | 手動紐付け | `esync link` で既存エンティティとローカルを接続 | ✅ |
| 中 | FR-21 | 整合性チェック | `esync verify` でローカル/リモートの整合性確認 | ✅ |
