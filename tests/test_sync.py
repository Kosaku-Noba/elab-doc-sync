...
# S-57: _parse_image_local_name の動作確認
def test_parse_image_local_name():
    assert _parse_image_local_name("items_42_photo.png") == "photo.png"
    assert _parse_image_local_name("experiments_1_fig.jpg") == "fig.jpg"
    assert _parse_image_local_name("photo.png") is None
    assert _parse_image_local_name("items_notanumber_photo.png") is None
...
# S-59: list_uploads 失敗時に warning が出力される（entity/entity_id 含む）
def test_download_images_warns_on_failure(capsys):
    ...
    assert "添付ファイル一覧の取得に失敗" in out
    assert "items #1" in out


# S-60: _normalize_remote_image_urls 失敗時に warning が出力される（entity/entity_id 含む）
...
# S-61: 再アップロード時にプレフィックスが除去された real_name でアップロードされる
def test_rewrite_images_upload_strips_prefix(tmp_path):
    docs = tmp_path / "docs"
    docs.mkdir()
    img_dir = docs / "images"
    img_dir.mkdir()
    (img_dir / "items_1_photo.png").write_bytes(b"\x89PNG_NEW")  # 8 bytes, サイズ不一致
    client = MagicMock()
    client.base_url = "https://elab.example.com"
    client.list_uploads.return_value = [
        {"id": 10, "real_name": "photo.png", "long_name": "abc123.png", "storage": "1", "filesize": 4},
    ]
    client.upload_file.return_value = {"url": "https://elab.example.com/dl/photo.png"}
    result = _rewrite_images("![a](images/items_1_photo.png)", "items", 1, client, docs, tmp_path)
    # upload_file が呼ばれ、アップロードされたファイル名が real_name (photo.png) であること
    client.upload_file.assert_called_once()
...
def test_merge_body_format_md(tmp_path, mock_client):
    ...
    docs.mkdir()
    (docs / "a.md").write_text("# hello", encoding="utf-8")
    target = TargetConfig(title="Test", docs_dir="docs/", id_file=str(tmp_path / ".ids" / "default.id"), body_format="md")
...
# S-81: merge — body_format=html で HTML 変換される
def test_merge_body_format_html(tmp_path, mock_client):
    docs = tmp_path / "docs"
    docs.mkdir()
    (docs / "a.md").write_text("# hello", encoding="utf-8")
    target = TargetConfig(title="Test", docs_dir="docs/", id_file=str(tmp_path / ".ids" / "default.id"), body_format="html")
    syncer = DocsSyncer(mock_client, target, tmp_path)
    mock_client.create_item.return_value = 1
...
def test_each_body_format_md(tmp_path, mock_client):
    ...
    docs.mkdir()
    (docs / "a.md").write_text("# hello", encoding="utf-8")
    target = TargetConfig(title="", docs_dir="docs/", id_file=str(tmp_path / ".ids" / "default.id"), mode="each", body_format="md")
    syncer = EachDocsSyncer(mock_client, target, tmp_path)
    mock_client.create_item.return_value = 1
    mock_client.get_item.return_value = {"id": 1, "body": "# hello"}
    syncer.sync()
    call_kwargs = mock_client.update_item.call_args[1]
...
# S-83: each — body_format=html で HTML 変換される
def test_each_body_format_html(tmp_path, mock_client):
    docs = tmp_path / "docs"
    docs.mkdir()
    (docs / "a.md").write_text("# hello", encoding="utf-8")
    target = TargetConfig(title="", docs_dir="docs/", id_file=str(tmp_path / ".ids" / "default.id"), mode="each", body_format="html")
    syncer = EachDocsSyncer(mock_client, target, tmp_path)
    mock_client.create_item.return_value = 1
    mock_client.get_item.return_value = {"id": 1, "body": "<h1>hello</h1>"}
    syncer.sync()
